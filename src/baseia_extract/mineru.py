...
def _read_json(request: urllib.request.Request, timeout: float) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            f"MinerU respondeu HTTP {error.code}: {request.full_url}. {body[:1000]}"
        ) from error
...
def _multipart(pdf_path: Path, fields: dict[str, str]) -> tuple[bytes, str]:
...
def _submit(pdf_path: Path, api_url: str) -> dict[str, Any]:
...
def _download(api_url: str, task_id: str) -> dict[str, Any]:
    request = urllib.request.Request(
...
def _save_result(result: dict[str, Any], output_dir: Path) -> None:
    ...
    for document_name, document_result in results.items():
        ...
        if isinstance(md_content, str):
            (document_dir / f"{document_name}.md").write_text(md_content, encoding="utf-8")
...
def _process(
    ...
    if not pdf_path.exists():
        return {**base, "status": "error", "error": "Arquivo PDF não encontrado"}
    if _completed(output_dir) and not config.overwrite:
        return {**base, "status": "skipped", "error": None}
    ...
    for attempt in range(config.retries + 1):
        ...
        try:
            with semaphores[api_url]:
                print(f"[{position + 1:04d}/{total:04d}] pod={pod_index + 1} | {pdf_path.name}")
                started = time.perf_counter()
                submission = _submit(pdf_path, api_url)
                task_id = submission.get("task_id")
                if not isinstance(task_id, str) or not task_id:
                    raise ValueError(f"Submissão sem task_id: {submission}")
                _wait(api_url, task_id)
                _save_result(_download(api_url, task_id), output_dir)
                return {
                    **base,
                    "status": "ok",
                    "task_id": task_id,
            ...
            errors.append(f"pod={pod_index + 1}: {type(error).__name__}: {error}")
...
def _execute(config: ExtractionConfig) -> dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=config.total_workers) as executor:
        futures = {
            ...
            for position, row in manifest_df.iterrows()
        }
        for future in as_completed(futures):
            runs.append(future.result())
            pd.DataFrame(runs).sort_values("document_position").to_csv(
                config.output_root / "runs.csv", index=False, encoding="utf-8-sig"
            )
    ...
    summary = {
        ...
        "skipped_count": int(runs_df["status"].eq("skipped").sum()),
        "error_count": int(runs_df["status"].eq("error").sum()),
        "elapsed_seconds": round(elapsed, 3),
        "pages_per_minute": round(float(pages) / elapsed * 60, 3) if elapsed else None,
    }
